...
class TestXTransformWrapper(unittest.TestCase):
    def test_seed(self):
        ...
        ds = XTransformWrapper(dataset=ds, transform=KDAdditiveGaussianNoise(std=1.), seed=5)
    ...
    def test_noseed(self):
        ds = XDataset(x=torch.randn(10, 5, generator=torch.Generator().manual_seed(5)))
        transform = KDAdditiveGaussianNoise(std=1.).set_rng(np.random.default_rng(seed=5))
